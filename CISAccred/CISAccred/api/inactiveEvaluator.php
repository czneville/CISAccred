<?php
	...
	if($verb == "get") 
	{
		...
		if($isActive == "0")
		{
			$query = executeQuery("SELECT * FROM CIS_EVALUATOR WHERE EVAL_ISACTIVE = 0 ORDER BY EVAL_ID ASC");
		}
		else
		{
			$query = executeQuery("SELECT * FROM CIS_EVALUATOR ORDER BY EVAL_ID ASC");
		}
